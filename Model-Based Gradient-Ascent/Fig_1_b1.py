...
if __name__ == "__main__":
    ...
    cal_grad = app.get_grad_cp(phases = phases)
    #####################################
    ...
    vmax = np.max(cal_grad) * 1.1
    vmin = np.min(cal_grad) * 0.9
    
    ax.plot(cal_grad, np.mean(est_grad, axis = 0), "ro")
    ax.ticklabel_format(axis='both', style='sci', scilimits=(0,0))
    ax.set_xlim(vmin, vmax)
    ax.set_ylim(vmin * perturbation_amplitude ** 2, vmax * perturbation_amplitude ** 2)
    ax.set_aspect(1e2)
    ax.plot(np.linspace(-1, 1, 1000), np.linspace(-1, 1, 1000) * perturbation_amplitude ** 2, ls = "--", alpha = 0.5, label = r"$y=σ^2\cdot x$")
    
    ax.legend(handletextpad=0.1, loc = "upper center")
    ax.set_xlabel("Calculated gradients (Eq.10)")
    ax.set_ylabel("Estimated SPGD update signals")
