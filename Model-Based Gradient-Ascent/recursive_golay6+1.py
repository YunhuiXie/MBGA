...
if __name__ == "__main__":
    ...
    d = 1
    R = 3 # increase this to play around with even more channels just for fun, but beware of RAM and VRAM usage
    
    lr = 1.5e2  # change this accordingly if "R" is changed
    
    s = 2
    xx, yy = np.meshgrid(np.linspace(-s, s, 1000), np.linspace(-s, s, 1000))

    def recursive_golay(R, d=1.0):
        ...
        # Golay-6 lattice points
        ij = np.array([[1,  0], [0, -1], [2, -1], [0, -2], [2, -3], [3, -3]])
        i, j = ij.T
    
        seed = np.column_stack((i + 0.5 * j, np.sqrt(3) / 2 * j))
        seed -= seed.mean(axis=0)
        seed = np.vstack((seed, [0.0, 0.0]))
        points = np.zeros((1, 2))
    
        for level in range(R):
            
            points = (points[None, :, :] + 4.0**level * seed[:, None, :]).reshape(-1, 2)
        points *= d
        
        return points[:, 0].tolist(), points[:, 1].tolist()
    
    x, y = recursive_golay(R, d)  # 49 beamlets
    L = min(1/(2 * (max(x) - min(x))), 1/(2 * (max(y) - min(y)))) # Eq. 12
                
    phase = np.random.uniform(low = -np.pi, high = np.pi, size = len(x))
    ...
    for _ in range(20):
        ...
        phase += app.get_grad_cp(phases = phase) * lr
        
        images.append(app.get_intensity(xx = xx, yy = yy, phases = phase))
    ...
    img = ax1.imshow(images[0], cmap = "jet", origin = "lower", vmin = 0, vmax = np.max(np.log(images)))
    
    frame_text = ax1.text(0.02, 0.98, f"Step 1 / {len(images)}", transform = ax1.transAxes, ha = "left", va = "top", color = "white", fontsize = 9,
                          bbox = dict(facecolor = "black", alpha = 0.65, edgecolor = "none", pad = 2))
    
    phases2 = np.mod(np.asarray(phases), 2 * np.pi)
    
    circles = []
    
    for _x, _y, _phi in zip(app.x, app.y, phases2[0]):
        
        c = Circle((_x, _y), 0.45, fill = True, linewidth = 2, fc = cmap(_phi / (2 * np.pi)))
        ax2.add_patch(c)
        circles.append(c)
    ...
    ax2.set_xlim(-(R*2)**2, (R*2)**2)
    ax2.set_ylim(-(R*2)**2, (R*2)**2)
    
    def update(frame):
    
        img.set_data(np.log(images[frame]))
        frame_text.set_text(f"Step {frame + 1} / {len(images)}")
        
        for circle, wrapped_phase in zip(circles, phases2[frame]):
            
            circle.set_facecolor(cmap(wrapped_phase / (2 * np.pi)))
    
        
        return (img, frame_text, *circles)
    
    ani = animation.FuncAnimation(fig = fig, func = update, frames = len(images), interval = 0.1)
    plt.show()