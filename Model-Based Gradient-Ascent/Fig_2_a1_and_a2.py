...
if __name__ == "__main__":
    ...
    phases = np.random.uniform(low = [x_lower, y_lower], high = [x_upper, y_upper], size = 2 )
    ...
    ax1.imshow(PIB_landscape, origin = "lower", cmap = "viridis")
    ax1.scatter( (phases[0] - x_lower) / (x_upper - x_lower) * sampling_size, 
                 (phases[1] - y_lower) / (y_upper - y_lower) * sampling_size,
                 c = "r", marker = "x", label = "you are here")
    
    ax2.imshow(app.get_intensity(xx = xx, yy = yy, phases = phases), origin = "lower", cmap = "jet")
    ax2.add_patch(patches.Rectangle((int((intensity_distance + 0 - L)/dl), 
                                     int((intensity_distance + 0 - L)/dl)),
                                    L*2/dl, L*2/dl,
                                    linewidth=0.5, linestyle = "-", edgecolor='r', facecolor='none', alpha = 1, label = "PIB window (Eq.12)"))
    
    ax1.legend()
    
    ax1.set_xticks([0, int(sampling_size/2 - 1), int(sampling_size-1)])
    ax1.set_yticks([0, int(sampling_size/2 - 1), int(sampling_size-1)])
    ax1.set_xticklabels([np.round(x_lower, 2), np.round((x_lower + x_upper)/2, 2), np.round(x_upper, 2)])
    ax1.set_yticklabels([np.round(y_lower, 2), np.round((y_lower + y_upper)/2, 2), np.round(y_upper, 2)])
    ax1.set_xlabel("Absolute phase " + r"$ϕ_1$")
    ax1.set_ylabel("Absolute phase " + r"$ϕ_2$")
    ax1.set_title("PIB landscape")
    
    ax2.set_xticks([])
    ax2.set_yticks([])
    ax2.set_title("Your far-field intensity distribution")