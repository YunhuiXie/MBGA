...
if __name__ == "__main__":
    ...
    cal_grads = app.get_grad_cp(phases = phases)
    #####################################
    ...
    for i in range(int(1e5)):
        ...
        est_grads.append((j2 - j1) * delta_phases)
    ###########################
    cal_vars = []
    for _grad in cal_grads:
        cal_vars.append( (np.sum(cal_grads ** 2) - _grad ** 2) * perturbation_amplitude ** 4 ) # Eq. 13

    est_vars = []
    for _var in np.var(est_grads, axis = 0):
        est_vars.append( _var )
        
    fig, ax = plt.subplots(1, 1, figsize = (6, 6), constrained_layout = True)
    
    vmax = np.max(cal_vars) * 1.1
    vmin = np.min(cal_vars) * 0.9
    
    ax.plot(cal_vars, est_vars, "ro")
    ax.ticklabel_format(axis='both', style='sci', scilimits=(0,0))
    ax.set_xlim(vmin, vmax)
    ax.set_ylim(vmin, vmax)
    ax.set_aspect(1)
    ax.plot(np.linspace(0, 1, 1000), np.linspace(0, 1, 1000), ls = "--", alpha = 0.5, label = r"$y=x$")
    
    ax.legend(handletextpad=0.1, loc = "upper center")
    ax.set_xlabel("Calculated variances (Eq.13)")
    ax.set_ylabel("Estimated SPGD update variances")
    
    ax.set_title(r"$R={:01d},σ^2={:.02f}$".format(R, perturbation_amplitude ** 2))