...
class GradCal:
    ...
    def get_PIB_analytic_batch(self,
                           phases_grid,
                           pib_index: int = 0,
                           *,
                           return_numpy: bool = True,):
    
        phi = cp.asarray(phases_grid, dtype=cp.float64)
        
        z = cp.exp(1j * phi)
        pw_table = self.pw_table_cp_list[pib_index]
    
        weighted = cp.matmul(cp.conj(z), pw_table.T)
        s_offdiag = cp.sum(z * weighted, axis=-1)
        total = (s_offdiag
                 + self.num_beamlets * self.pw0_cp_list[pib_index])
...
if __name__ == "__main__":
    ...
    phases1 = np.array([-0.40 * np.pi, 0.40 * np.pi])
    phases2 = np.array([-0.40 * np.pi, 0.40 * np.pi])
    phases3 = np.array([-0.40 * np.pi, 0.40 * np.pi])
    phases4 = np.array([-0.40 * np.pi, 0.40 * np.pi])
    
    phi1, phi2, phi3, phi4 = [phases1.copy()], [phases1.copy()], [phases1.copy()], [phases1.copy()]
    
    L = 1/2 # Eq. 12

    lr1 = 15.8 # these are step sizes
    lr2 = 26e3
    lr3 = 1.1
    lr4 = 0.95
    ...
    p1, p2 = np.meshgrid(np.linspace(x_lower, x_upper, sampling_size),
                         np.linspace(y_lower, y_upper, sampling_size),
                         indexing="ij")
    
    PIB_landscape = app.get_PIB_analytic_batch(phases_grid = np.stack([p1, p2], axis=-1))
    
    for _ in range(3): # MBGA
        
        phases1 += lr1 * app.get_grad_cp(phases1)
        phi1.append(phases1.copy())
        
    for _ in range(6): # SPGD. Again, results may not be identical to the paper due to how random number generator works
        
        delta_phases = np.random.choice(a = [-0.1, 0.1], size = 2)
        
        j1 = app.get_PIB_analytic(phases = phases2)
        j2 = app.get_PIB_analytic(phases = phases2 + delta_phases)
        
        phases2 += lr2 * (j2 - j1) * delta_phases
        
        phi2.append(phases2.copy())
        
    for _ in range(3): # Self-referenced
        
        phases3[0] += lr3 * np.sin(phases3[1] - phases3[0])
        
        phi3.append(phases3.copy())
        
    phi_ref = 0.25 * np.pi # target phases
    for _ in range(3): # external-referenced
        
        phases4 += lr4 * np.sin(phi_ref - phases4)
        phi4.append(phases4.copy())
        
    fig, [[ax1, ax2], [ax3, ax4]] = plt.subplots(2, 2, figsize = (6, 6), constrained_layout = True)
    
    for _ax, _list, _name in zip([ax1, ax2, ax3, ax4], [phi1, phi2, phi3, phi4], ["MBGA", "SPGD", "Self-referenced", "external-referenced"]):
        
        _ax.imshow(PIB_landscape, origin = "lower", cmap = "viridis")
        _ax.set_xticks([0, int(sampling_size/2 - 1), int(sampling_size-1)])
        _ax.set_yticks([0, int(sampling_size/2 - 1), int(sampling_size-1)])
        _ax.set_xticklabels([np.round(x_lower, 2), np.round((x_lower + x_upper)/2, 2), np.round(x_upper, 2)])
        _ax.set_yticklabels([np.round(y_lower, 2), np.round((y_lower + y_upper)/2, 2), np.round(y_upper, 2)])
        _ax.set_xlabel("Absolute phase " + r"$ϕ_1$")
        _ax.set_ylabel("Absolute phase " + r"$ϕ_2$")
        
        _x = (np.asarray(_list).T[0] - x_lower) / (x_upper - x_lower) * sampling_size
        _y = (np.asarray(_list).T[1] - y_lower) / (y_upper - y_lower) * sampling_size
        
        if _ax != ax2:
        
            _ax.scatter(_x, _y, c = "r", s = 7)
         
            for i in range(int(_x.shape[0]-1)):
            ...
            vals, counts = np.unique(_x, return_counts=True)
            unique_idx = np.flatnonzero(np.isin(_x, vals[counts == 1]))
            dup_idx = np.flatnonzero(np.isin(_x, vals[counts > 1]))

            for _idx in unique_idx:
                
                _ax.scatter(_x[_idx], _y[_idx], c = "r", s = 7)
                
            for _idx1, _idx2 in zip(dup_idx, np.linspace(-int(dup_idx.shape[0])/2, int(dup_idx.shape[0])/2, int(dup_idx.shape[0]))):
                
                _ax.scatter(_x[_idx1] + _idx2 * 30, _y[_idx1] + _idx2 * 30, c = "r", s = 7)
