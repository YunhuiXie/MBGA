...
if __name__ == "__main__":
    ...
    lr3 = 1
    distance = 0.14
    angle = np.linspace(0, 2*np.pi, 12)[:-1]
    ...
    app3 = GradCal(x=x,
                   ...
                   beamlet_waist = 0.2,
                   beamlet_amplitude = 1,
                   PIB_windows_size_x = [L3] * sine.shape[0], 
                   PIB_windows_size_y = [L3] * sine.shape[0], 
                   PIB_windows_displace_x = distance * cosine, 
                   PIB_windows_displace_y = distance * sine,
                   n_workers=None,
                   chunk_size=1024)
    
    vmax = app1.get_intensity(xx = xx, yy = yy, phases = np.zeros(shape = len(x))).max()
    ...
    ax1.imshow(app1.get_intensity(xx = xx, yy = yy, phases = phases1), cmap = "jet", origin = "lower", vmin = 0, vmax = vmax)
    ax2.imshow(app2.get_intensity(xx = xx, yy = yy, phases = phases2), cmap = "jet", origin = "lower", vmin = 0, vmax = vmax)
    ax3.imshow(app3.get_intensity(xx = xx, yy = yy, phases = phases3), cmap = "jet", origin = "lower", vmin = 0, vmax = vmax)
    
    ax1.add_patch(patches.Rectangle((int((intensity_distance - L1)/dl),
                                     int((intensity_distance - L1)/dl)),
                                    L1*2/dl, L1*2/dl, linewidth=1, linestyle = "-", edgecolor='r', facecolor='none', alpha = 1, label = "Eq.12"))
    
    ax2.add_patch(patches.Rectangle((int((intensity_distance + lx_offset - L2)/dl),
                                     int((intensity_distance + ly_offset - L2)/dl)),
                                    L2*2/dl, L2*2/dl, linewidth=1, linestyle = "-", edgecolor='r', facecolor='none', alpha = 1, label = "Eq.12"))
    
    for idx, (_x, _y) in enumerate(zip(distance * cosine, distance * sine)):
        
        if idx == 0:
            
            ax3.add_patch(patches.Rectangle((int((intensity_distance + _y - L3)/dl),
                                             int((intensity_distance + _x - L3)/dl)),
                                            L3*2/dl, L3*2/dl, linewidth=1, edgecolor='r', facecolor='none', label = "PIBs"))
        else:
            
            ax3.add_patch(patches.Rectangle((int((intensity_distance + _y - L3)/dl),
                                             int((intensity_distance + _x - L3)/dl)),
                                            L3*2/dl, L3*2/dl, linewidth=1, edgecolor='r', facecolor='none'))
    ...
    for _ax, _phi in zip([ax1, ax2, ax3], [phases1, phases2, phases3]):
        ...
        for idx, (_x, _y) in enumerate(zip(x, y)):
            ...
            c = patches.Circle((_x, _y), 0.45, fill=True, linewidth=2, fc = _arr_phi[idx])
            _ax_0.add_patch(c)
