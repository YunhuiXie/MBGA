...
class GradCal:
    ...
    def _build_pwz_table(self,
        ...
        pwz = (axis_factor(x_left, x_right, Lx, lx)
               * axis_factor(y_left, y_right, Ly, ly))
        pwz = 0.5 * (pwz + pwz.conj().T)
        pw0 = complex(np.trace(pwz).real / self.num_beamlets)
        np.fill_diagonal(pwz, 0.0)
    ...
    def _build_pw_table(self,
        ...
        if self.off_fourier_plane:
            ...
            return self._build_pwz_table(Lx, Ly, lx, ly,)
