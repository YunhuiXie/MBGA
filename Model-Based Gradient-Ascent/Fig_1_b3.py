...
class GradCal:
    ...
    def __init__(self,
        ...
        self.grad_prefix = -(self.A * self.A * self.w * self.w * np.pi) / 4.0
...
if __name__ == "__main__":
    ...
    for R in [1, 2, 3, 4, 5, 6]:
        ...
        phases = np.random.uniform(low = -np.pi, high = np.pi, size = len(x))
        ...
        ### MBGA direct phase calculation ###
        cal_grads.append(app.get_grad_cp(phases = phases))
        #####################################
        
        ### SPGD phase estimate ###
        est_grad = []
        perturbation_amplitude = 0.1 # you can play around with this but doing so may break gradient estimate
    
        for i in range(int(1e5)):
            
            delta_phases = np.random.choice(a = [-perturbation_amplitude, perturbation_amplitude], size = len(x))
            j1 = app.get_PIB_analytic(phases = phases)
            j2 = app.get_PIB_analytic(phases = phases + delta_phases)
            
            est_grad.append((j2 - j1) * delta_phases)
        ###########################
        
        est_grads.append(est_grad)

    cal_snr_collection = []
    for cal_grad in cal_grads:
        _cal_snrs = []
        for _grad in cal_grad:
            _cal_snrs.append( np.sqrt(_grad ** 2 / (np.sum(cal_grad ** 2) - _grad ** 2)) ) # Eq. 14
        cal_snr_collection.append(_cal_snrs)
        
    est_snr_collection = []
    for est_grad in est_grads:
        _est_snrs = []
        for _grad, _var in zip(np.mean(est_grad, axis = 0), np.var(est_grad, axis = 0)):
            _est_snrs.append( np.abs(_grad) / np.sqrt(_var) ) # by definition
        est_snr_collection.append(_est_snrs)
            
    fig, ax = plt.subplots(1, 1, figsize = (5, 5), constrained_layout = True)
    
    for idx, (cal_snrs, est_snrs) in enumerate(zip(cal_snr_collection, est_snr_collection)):
        
        cal_med = np.median(cal_snrs)
        est_med = np.median(est_snrs)
        
        cal_q1, cal_q3 = np.percentile(cal_snrs, [25, 75])
        est_q1, est_q3 = np.percentile(est_snrs, [25, 75])
        
        cal_yerr = [[cal_med - cal_q1], [cal_q3 - cal_med]]
        est_yerr = [[est_med - est_q1], [est_q3 - est_med]]
        
        if idx == 0:
        
            ax.errorbar(idx - 0.125 + 1, [cal_med], yerr = cal_yerr, fmt='o', markersize = 4, capsize=2, c = "r", label = "Eq.14")
            ax.errorbar(idx + 0.125 + 1, [est_med], yerr = est_yerr, fmt='x', markersize = 4, capsize=2, c = "b", label = "Estimated")
            
        else:
            
            ax.errorbar(idx - 0.125 + 1, [cal_med], yerr = cal_yerr, fmt='o', markersize = 4, capsize=2, c = "r")
            ax.errorbar(idx + 0.125 + 1, [est_med], yerr = est_yerr, fmt='x', markersize = 4, capsize=2, c = "b")
            
    ax.plot(np.linspace(1, 6, 6), 1/np.sqrt(3 * np.linspace(1, 6, 6) * (np.linspace(1, 6, 6) + 1)), 
            label = r"$\frac{1}{\sqrt{3 \cdot R \cdot (R+1)}}$",
            ls = "--",
            c = "green")
        
    ax.set_xlabel(r"Array size $R$")
    ax.set_ylabel("SPGD signal-to-noise ratio")
    
    ax.set_ylim(0, 0.5)
    ax.set_xlim(0.5, 6.5)
    
    ax.legend(ncol = 1, columnspacing=0.05, handletextpad=0.05, loc = "upper right")
